...
define(['moment', 'underscore','app/d3.chart.analog', 'app/d3.chart.digital', 'app/d3.chart.horizon','app/d3.chart.scatter'], function (moment) {
    ...
    var logChartHeight = 200, diChartHeight = 20;
    var gap = 30;
    ...
    function selectChart(d) {
        if (d.type == 'analog') {
            ...
            chart = d3.scatter().height(logChartHeight).gap(gap).color(color);
        }
    ...
    function graphHeight(d) {
        if (d.type == 'analog' || d.type == 'scatter')
    ...
    this.addGraph = function (graph) {        
        ...
        var brush = d3.svg.brush()
            ...
            .on('brushend', function () { 
                ...
                d3.selectAll(".dot2").transition().duration(1600).attr("cx",function(d){ return _xScale(moment(d.DateTime));});
                //.attr("cy", function(d){ return _yScale(d.value);});
                localStorage.setItem("min_date",brush.extent()[0]);
                localStorage.setItem("max_date",brush.extent()[1]);
                //$('.loader').show();
...
function displayStorageEvent(e) {
                if (e.key == 'min_date') {
                  var d=new Date(localStorage['min_date'])
                  var year=d.getFullYear();
                  console.log(year.toString())
                      // define our brush extent to be begin and end of the year
                  brush.extent([d, new Date(localStorage['max_date'])])
